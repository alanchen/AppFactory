...
- (AFTImageScrollView *)pageForIndex:(NSInteger)index {
    NSArray *subviews = [self.pagingScrollView subviews];
    for (AFTImageScrollView *page in subviews) {
        if([page isKindOfClass:[AFTImageScrollView class]]){
            if (page.pageIndex == index) {
                return  page;
            }
        }
