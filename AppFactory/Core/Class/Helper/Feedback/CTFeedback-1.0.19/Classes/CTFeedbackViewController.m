...
@interface CTFeedbackViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
{
    [self.contentCellItem.textView resignFirstResponder]; //added by Redbug
...
- (NSArray *)additionCellItems{
    ...
    self.additionCellItem.action = ^(CTFeedbackViewController *sender){
        ...
        [actionSheet addCancelActionWithTitle:CTFBLocalizedString(@"Cancel") handler:nil];
        [actionSheet addDefaultActionWithTitle:CTFBLocalizedString(@"Camera") handler:^(UIAlertAction *action) {
            UIImagePickerControllerSourceType sourceType = UIImagePickerControllerSourceTypePhotoLibrary;
            if([weakSelf isCameraAvailable]) {sourceType = UIImagePickerControllerSourceTypeCamera;}
            [weakSelf createImagePickerControllerWithSourceType:sourceType];

        }];
        
        [actionSheet addDefaultActionWithTitle:CTFBLocalizedString(@"PhotoLibrary") handler:^(UIAlertAction *action) {
            UIImagePickerControllerSourceType sourceType = UIImagePickerControllerSourceTypePhotoLibrary;
            [weakSelf createImagePickerControllerWithSourceType:sourceType];
        }];
        
        UITableViewCell *cell = [weakSelf.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:1]];
        [actionSheet showActionSheetOnViewController:weakSelf from:cell];
    };
...
- (NSString *)platformString
{
    ...
    return platformString?platformString:@"";
}
