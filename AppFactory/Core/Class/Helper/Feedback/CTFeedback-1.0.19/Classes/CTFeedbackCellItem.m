...
- (id)init
{
    self = [super init];
    if (self) {
        self.textView = [[UITextView alloc] initWithFrame:CGRectMake(10, 20, 310, ContentCellHeight)];
        ...
        [self.textLabel setText:CTFBLocalizedString(@"Please leave your feedback below:")];
        [self.textLabel sizeToFit];
        [self.textLabel setFrame:CGRectMake(15, 5, self.textLabel.bounds.size.width, self.textLabel.bounds.size.height)];
...
- (CGFloat)cellHeight
{
    return MAX(ContentCellHeight, self.textView.contentSize.height + self.textLabel.frame.size.height);
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    frame.size.height = MAX(ContentCellHeight - self.textLabel.frame.size.height, self.textView.contentSize.height);
